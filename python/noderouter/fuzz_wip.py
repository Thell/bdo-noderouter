...
# A simple counter for instances with cost mistmatch but correct ccs
repairable_via_steiner_tree = 0
...
class _FuzzInstanceMetrics(dict):
    ...
    def __init__(
        ...
        assert ratio >= 1.0, f"NodeRouter should never have lower cost than MIP! {seed}"
        assert speedup > 1.0, f"NodeRouter should always be faster than MIP! {seed}"

        self["cc_status"] = self._validate_components(nr_instance, mip_instance)
    ...
    def _validate_components(self, nr_instance, mip_instance) -> int:
        """
        Compare NR and MIP terminal/root SCC partitions and classify mismatch type.

            0 = perfect match
            1 = NR merged components that MIP kept separate
            2 = NR split components that MIP kept together
            3 = mixed merge/split or structurally incompatible
        """
        import rustworkx as rx

        from api_exploration_data import ExplorationData, get_exploration_data
        from api_rx_pydigraph import subgraph_stable

        def extract_components(instance: Instance):
            assert instance.solution
            exp: ExplorationData = get_exploration_data()
            G = exp.super_graph
            node_map = G.attrs["node_key_by_index"]  # bidict (index -> waypoint key)

            # Build solution subgraph from waypoint indices
            sol_node_indices = [node_map.inv[w] for w in instance.solution.waypoints]
            sol_graph = subgraph_stable(G, sol_node_indices)

            # SCCs of the solution subgraph
            sccs = rx.strongly_connected_components(sol_graph)

            # Terminal + root node indices
            terminal_idx = {node_map.inv[w] for w in instance.terminals.terminals}
            root_idx = {node_map.inv[w] for w in instance.terminals.terminals.values()}
            tr_idx = terminal_idx | root_idx

            # Reduce SCCs to only terminals+roots
            reduced = []
            for comp in sccs:
                subset = tr_idx.intersection(comp)
                if subset:
                    reduced.append(frozenset(subset))

            # Deterministic ordering
            reduced = sorted(reduced, key=lambda s: (len(s), tuple(sorted(s))))

            return reduced

        nr_comps = extract_components(nr_instance)
        mip_comps = extract_components(mip_instance)

        # Perfect match
        if nr_comps == mip_comps:
            return 0

        # Detect merges/splits
        def overlaps(a, b):
            return not a.isdisjoint(b)

        # NR merged if one NR comp overlaps multiple MIP comps
        nr_merged = any(sum(1 for d in mip_comps if overlaps(c, d)) > 1 for c in nr_comps)

        # NR split if one MIP comp overlaps multiple NR comps
        nr_split = any(sum(1 for c in nr_comps if overlaps(d, c)) > 1 for d in mip_comps)

        if nr_merged and nr_split:
            return 3
        if nr_merged:
            return 1
        if nr_split:
        ...
        # Fallback: structurally incompatible
        return 3
...
def _run_single_config(
    ...
    global repairable_via_steiner_tree
    starting_repairable_via_steiner_tree = repairable_via_steiner_tree
    ...
    for strategy in strategies:
        ...
        if len(case_rows) == 0:
            logger.warning(f"Skipping: {desc}, samples={samples}")
            continue
        ...
        if repairable_via_steiner_tree != starting_repairable_via_steiner_tree:
            logger.warning(
                f"Repairable: {repairable_via_steiner_tree - starting_repairable_via_steiner_tree}"
            )
...
def fuzzer_main(strategies: list[PairingStrategy], samples: int, budgets: list[int] | range) -> None:
    ...
    try:
        ...
        if repairable_via_steiner_tree > 0:
            logger.error(f"Repairable: {repairable_via_steiner_tree}")
