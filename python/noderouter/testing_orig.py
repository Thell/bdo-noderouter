...
def _generate_random_terminals(
    ...
    selected_nodes = _select_nodes(worker_nodes, num_workers, seed)
    ...
    src_dst: dict[int, int] = {
        **_assign_worker_roots(selected_nodes, exploration_data, config.get_territory_root_sets(), seed),
    ...
    for cost in range(5, 555, 5):
        for include_danger in [False, True]:
            ...
            logger.info(
                ...
                f"Roots: {root_count}, Workers: {worker_count}, Dangers: {danger_count}\nTerminals: {src_dst}"
            )
    ...
    for worker_count in [1, 3, 5, 10, 20, 30, 50, 75, 100]:
        for include_danger in [False, True]:
            ...
            logger.info(
                ...
                f"Roots: {root_count}, Workers: {wc_out}, Dangers: {danger_count}\nTerminals: {src_dst}"
            )
