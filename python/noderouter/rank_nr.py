...
# There are 3 Algorithms with 4 recorded solution costs for each test.
...
#
# For example the following: pre_iop_fwd,iop_fwd,pre_iop_rev,iop_rev are for one algorithm during one test
# The 3 values per algorithm for reporting are: pre_iop_fwd, iop_fwd, iop_rev
#
# The reported columns names should be transformed to: XXX_raw, XXX_fwd, XXX_rev

# CVS Rank columns are:
# pre_iop_fwd,iop_fwd,pre_iop_rev,iop_rev,pre_gssp_fwd,gssp_fwd,pre_gssp_rev,gssp_rev,pre_pd_fwd,pd_fwd,pre_pd_rev,pd_rev
...
file_path = ds.path().joinpath("rank_results.csv")
...
file_path = ds.path().joinpath("bench_results.csv")
...
alg_long = alg_long.with_columns([pl.col("metric").str.extract(r"(iop|gssp|pd)").alias("algorithm")])
...
def rank_composition_band(band_df: pl.DataFrame) -> pl.DataFrame:
    ...
    for r in [1, 2, 3]:
        masks = (
            ...
            .with_columns([
                pl
                .when((pl.col("algorithm") == "iop") & (pl.col("pre_rank") == r))
                .then(1)
                ...
                .alias("pre_bit"),
                pl
                .when((pl.col("algorithm") == "iop") & (pl.col("fwd_rank") == r))
                .then(1)
                ...
                .alias("fwd_bit"),
                pl
                .when((pl.col("algorithm") == "iop") & (pl.col("rev_rank") == r))
                .then(1)
                ...
                .alias("rev_bit"),
                pl
                .when((pl.col("algorithm") == "iop") & (pl.col("final_rank") == r))
                .then(1)
...
def placement_table_strategy(df: pl.DataFrame) -> pl.DataFrame:
    ...
    seed = pl.DataFrame(
        [(alg, r) for alg in ["iop", "gssp", "pd"] for r in [1, 2, 3]],
...
improved = improved.with_columns([
    ((pl.col("algorithm") == "iop") & pl.col("improved_flag")).alias("iop_imp"),
...
for alg in ["iop", "gssp", "pd"]:
    ...
    if subset.is_empty():
        magn_rows.append((alg.upper(), 0, 0, 0, 0, 0, 0, 0, 0, 0))
        continue
...
print_summary(
    "STRATEGY RANK‑1 DOMINANCE",
    strategy_rank1_all.filter(pl.col("algorithm") == "iop").sort(
        ["coverage_band", "final_rank1_percent"], descending=[False, True]
    ),
)
print_summary(
...
# Drop IOP for more meaningful plot between GSSP and PD.
df_long = df_long.filter(pl.col("algorithm") != "iop")

# Limit to Budgets < 200
df_long = df_long.filter(pl.col("budget") < 200)
...
plt.savefig("strategy_transitions_cmp_extended_bridge.png", bbox_inches="tight")
